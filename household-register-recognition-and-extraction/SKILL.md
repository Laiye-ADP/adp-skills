---
name: household-register-recognition-and-extraction
...
能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=github)提供，支持中国户口本的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成户口本图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。
...
| 识别 Base64 编码的户口本 | `adp extract base64 <base64> --app-id <户口本抽取应用ID> --file-name <文件名.后缀>` |
...
```bash
adp extract base64 <base64字符串> --app-id <户口本抽取应用ID> --file-name <文件名.后缀>
```
