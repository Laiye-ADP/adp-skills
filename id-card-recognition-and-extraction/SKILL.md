---
name: id-card-recognition-and-extraction
...
能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=github)提供，支持中国居民身份证的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成身份证图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张身份证**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。立即登录：[中国大陆](https://adp.laiye.com/?utm_source=github) | [海外地区](https://adp-global.laiye.com/?utm_source=github)
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张身份证图片（URL） | `adp extract url <URL> --app-id <身份证抽取应用ID>` |
| 识别一张本地身份证图片 | `adp extract local <文件路径> --app-id <身份证抽取应用ID>` |
| 识别 Base64 编码的身份证 | `adp extract base64 <base64> --app-id <身份证抽取应用ID> --file-name <文件名.后缀>` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的身份证 | `adp extract local <文件夹路径> --app-id <身份证抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <身份证抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <身份证抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <身份证抽取应用ID> --async` |
...
```bash
adp extract local ./id-card-front.jpg --app-id <身份证抽取应用ID>
```

### 单张身份证抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <身份证抽取应用ID> --file-name <文件名.后缀>
```
...
```bash
adp extract local ./id-cards/ --app-id <身份证抽取应用ID> --export ./results/ 
```
...
```bash
# 提交异步任务
adp extract url https://example.com/id-card.jpg --app-id <身份证抽取应用ID> --async
...
| 查询余额 | `adp credit` |
| 充值方式 | 登录 ADP 门户网站充值：[中国大陆及港澳台地区](https://adp.laiye.com/?utm_source=github) \| [非中国大陆及港澳台地区](https://adp-global.laiye.com/?utm_source=github) |
...
如需了解完整能力，请访问：
- ADP 中国大陆：[https://adp.laiye.com/](https://adp.laiye.com/?utm_source=github)
- ADP 非中国大陆：[https://adp-global.laiye.com/](https://adp-global.laiye.com/?utm_source=github)
