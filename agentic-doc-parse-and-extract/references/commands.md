...
Manage review rules and tasks for post-extraction quality control. Each application (`--app-id`) can have one associated human-review configuration.

### `adp human-review rule-create`

Create a human-review rule for an application.

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--app-id` | string | **Yes** | — | Application ID (serves as unique identifier for the review configuration) |
| `--rule-name` | string | **Yes** | — | Rule name (must be unique within the application) |
| `--rule-status` | boolean | No | true | Enable (`true`) or disable (`false`) the rule |
| `--rule` | JSON array | **Yes** | — | Review rules (max 100). Each element: `{"rule_dimension": "<dimension>", "rule_setting": "<setting>"}` |
| `--rule-logic` | integer | No | 1 | Combination logic when multiple rules are set: `1` = any condition triggers review, `2` = all conditions must match |

**Rule dimensions**: full document, document confidence, or any extraction field name.
**Rule settings**: field not empty, maximum amount limit, specific time range, etc.

### `adp human-review get-config`

View the current human-review configuration for an application.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--app-id` | string | **Yes** | Application ID |

### `adp human-review rule-update`

Update an existing human-review rule.

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--app-id` | string | **Yes** | — | Application ID |
| `--rule-name` | string | **Yes** | — | Rule name (must be unique within the application) |
| `--rule-status` | boolean | No | false | Enable (`true`) or disable (`false`) the rule |
| `--rule` | JSON array | **Yes** | — | Updated review rules (max 100). Each element: `{"rule_dimension": "<dimension>", "rule_setting": "<setting>"}` |
| `--rule-logic` | integer | No | 1 | Combination logic: `1` = any condition, `2` = all conditions |

### `adp human-review rule-delete`

Delete all human-review rules for an application.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--app-id` | string | **Yes** | Application ID |

### `adp human-review rule-ai-generate`

AI-powered review rule recommendation based on the application's extraction configuration.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--app-id` | string | **Yes** | Application ID |

### `adp human-review task-create`

Create a human-review task to process documents with review rules applied. Defaults to synchronous processing; use `--async` for asynchronous mode.

| Option | Type | Required | Default | Description |
|--------|------|----------|---------|-------------|
| `--app-id` | string | **Yes** | — | Application ID |
| `--local` | string | No | — | Local file path or folder path (batch mode when folder) |
| `--url` | string | No | — | File URL, or path to a text file containing a list of URLs (batch mode) |
| `--async` | boolean | No | false | Enable asynchronous processing; returns a task ID for later querying |

> Note: Provide exactly one of `--local` or `--url`.

### `adp human-review task-query <task-id>`

Query the status and results of an asynchronous human-review task.

| Argument | Type | Required | Description |
|----------|------|----------|-------------|
| `<task-id>` | string | **Yes** | Task ID returned by `task-create --async` |

### `adp human-review result-update`

Update the reviewed field values for a document in a human-review task. Only documents with pending status can be updated.

| Option | Type | Required | Description |
|--------|------|----------|-------------|
| `--file-task-id` | string | **Yes** | Document task ID of the file to update |
| `--collaboration-result` | JSON array | **Yes** | Updated field values. Each element: `{"field_name": "<name>", "field_type": "<type>", "field_values": "<value>"}` |
